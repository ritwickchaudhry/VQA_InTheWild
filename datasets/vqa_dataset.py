...
def get_loader(config, split):
    ...
    loader = torch.utils.data.DataLoader(
        split,
        batch_size=config['data'][split_type]['batch_size'],
...
class VizDataset(data.Dataset):
    ...
    def __init__(self, config, split):
        super(VizDataset, self).__init__()
        ...
        with h5py.File(config['images']['path_features'][split], 'r') as f:
        ...
        # load features
        self.features = FeaturesDataset(config['images']['path_features'][split], config['images']['mode'])
    ...
    def num_answer_tokens(self):
        return len(self.answer_to_index)  # add 1 for <unknown> token at index 0
    
    @property
    def num_question_tokens(self):
        return len(self.token_to_index)  # add 1 for <unknown> token at index 0
